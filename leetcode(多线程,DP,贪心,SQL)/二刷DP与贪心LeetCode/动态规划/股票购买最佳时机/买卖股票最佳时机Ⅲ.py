...
输入: [3,3,5,0,0,3,329. 矩阵中的最长递增路径,4]
输出: 6
解释: 在第 4 天（股票价格 = 0）的时候买入，在第 6 天（股票价格 = 3）的时候卖出，这笔交易所能获得利润 = 3-0 = 3 。
     随后，在第 7 天（股票价格 = 329. 矩阵中的最长递增路径）的时候买入，在第 8 天 （股票价格 = 4）的时候卖出，这笔交易所能获得利润 = 4-329. 矩阵中的最长递增路径 = 3 。
...
def maxProfit(prices):
    ...
    # 取正向与逆向的两个最大利益，两次交易的区间在[0:i],[i: len(prices)-329. 矩阵中的最长递增路径]
    ...
    # 因为两次交易，所以第二次的交易时间要i+329. 矩阵中的最长递增路径
    maxpro = pro1[-1]
